...
namespace PESStadiums
{
    ...
    public struct _vertex
    {
        ...
        public void add(string[] stable)
        {
            ...
            else if (stable.Length == 8)
            {
                ...
                y = System.Convert.ToSingle(stable[1], nfi);
                z = System.Convert.ToSingle(stable[2], nfi);
                nx = System.Convert.ToSingle(stable[3], nfi);
                ny = System.Convert.ToSingle(stable[4], nfi);
                nz = System.Convert.ToSingle(stable[5], nfi);
                u = System.Convert.ToSingle(stable[6], nfi);
                v = System.Convert.ToSingle(stable[7], nfi);
            }
            else if (stable.Length == 10)
            {
                NumberFormatInfo nfi = new CultureInfo("en-US", false).NumberFormat;
                nfi.NumberDecimalSeparator = ".";
                x = System.Convert.ToSingle(stable[0], nfi);
                ...
                u = System.Convert.ToSingle(stable[6], nfi);
                v = System.Convert.ToSingle(stable[7], nfi);
                u2 = System.Convert.ToSingle(stable[8], nfi);
                v2 = System.Convert.ToSingle(stable[9], nfi);
            }
        }
    ...
    public struct _simplevert
    {
        ...
        public void add(string[] stable)
        {
            ...
            y = System.Convert.ToSingle(stable[1], nfi);
            z = System.Convert.ToSingle(stable[2], nfi);
            u = Single.Parse(stable[3], nfi);
            v = Single.Parse(stable[4], nfi);
        }
    }
    ...
    public class Tools
    {
        ...
        public void decompresstoMemory(string inFile, MemoryStream memorystream)
        {
            ...
            byte[] buffer = new byte[instream.Length - 16 - zeros];
            instream.Read(buffer, 0, buffer.Length);
            zlibstream.Write(buffer, 0, buffer.Length);
        ...
        public MemoryStream StreamtoMemory(Stream instream)
        {
            int size;
        ...
        public _stadium readKTMDL(Stream[] files, bool trianglestrip)
        {
            ...
            for (int t = 0; t < files.Length; t++)
            {
                ...
                for (int a = 0; a < sections; a++)
                {
                    ...
                    ratio = offsets[a].magic;
                    if (textur.Length > 0)
                        model.meshes[t].objs[a].matname = textur[section_mat[a]];
                    else
                        model.meshes[t].objs[a].matname = "0000";
        ...
        public _stadium readSef2010(string filename)
        {
            _stadium tempstadium = new _stadium();

            string line;
            tempstadium.crowd.sides = new _side[20];
            int meshescount, matscount, textscount, crowdside;
            ...
            if (input.ReadLine() != "//2010 format by liberostelios")
                throw new Exception("This is not a SEF 2010 stadium file.");
            ...
            matscount = Convert.ToInt32(line.Split(' ')[2]);
            tempstadium.materials = new _material[matscount];
            ...
            {
                tempstadium.materials[i].name = line.Split(' ')[0];
                tempstadium.materials[i].texs = new short[line.Split(' ').Length - 1];
                for (int j = 1; j < line.Split(' ').Length; j++)
                    tempstadium.materials[i].texs[j - 1] = (short)int.Parse(line.Split(' ')[j], System.Globalization.NumberStyles.HexNumber, null);
                line = input.ReadLine();
            }
            ...
            {
                line = input.ReadLine();
            } while (!line.StartsWith("Textures"));
            ...
            for (int i = 0; i < meshescount; i++)
            {
                ...
                {
                    line = input.ReadLine();
                    tempstadium.meshes[i].objs[j].matname = line.Split(' ')[1];
                    line = input.ReadLine();
                    tempstadium.meshes[i].objs[j].vertcount = int.Parse(line.Split(' ')[0]);
                    tempstadium.meshes[i].objs[j].uvs = int.Parse(line.Split(' ')[1]);
        ...
        public Stream makepartfilefull(_mesh mymesh, _stadium stadium, bool backside)
        {
            byte[] buffer;
            ...
            List<_material> mats = new List<_material>();
            _material m;
            List<_texture> texs = new List<_texture>();

            foreach (_material mat in stadium.materials)
                foreach (_object obj in mymesh.objs)
                    if (obj.matname == mat.name)
                    {
                        mats.Add(mat);
                        foreach (short texid in mat.texs)
                            foreach (_texture tex in stadium.textures)
                                if (tex.id == texid)
                                {
            ...
            binw.Write(mymesh.objcount); //40
            binw.Write(448);
            binw.Write(mymesh.objcount * 2);
            section = 448 + mymesh.objcount * 96;
            binw.Write(section); //52
            binw.Write(0);
            binw.Write(448); //60
            binw.Write(1);
            section += mymesh.objcount * 64 + 16;
            binw.Write(section);
            binw.Write(mymesh.objcount); //72
            section += 144;
            binw.Write(section);
            binw.Write(texs.Count); //80
            section += mymesh.objcount * 160 + texs.Count * 16;
            binw.Write(section); //84
            binw.Write(1); //88 
            binw.Write(section - mymesh.objcount * 160 - texs.Count * 16 - 48); //92
            section += 80 * texs.Count;
            binw.Write(section); //96
            binw.Write(192); //100
            binw.Write(0); //104
            section = 448 + mymesh.objcount * 96;
            binw.Write(section); //108
            section += mymesh.objcount * 64;
            binw.Write(section); //112
            binw.Write(0);//Change later - vertex data size //116
            binw.Write(0);//Change later - 1st vertex data offset //120
            binw.Write(texs.Count); //124
            ...
            for (i = 0; i < mymesh.objcount; i++)
            {
                ...
                binw.Write(0);
                m = mats.Find(delegate(_material mat) { return mat.name == mymesh.objs[i].matname; });
                binw.Write(m.texs.Length);
                binw.Write((short)texs.IndexOf(texs.Find(delegate(_texture tex) { return tex.id == m.texs[0]; })));
                if (m.texs.Length > 1)
                {
                    binw.Write((short)1);
                    binw.Write((short)texs.IndexOf(texs.Find(delegate(_texture tex) { return tex.id == m.texs[1]; })));
                    binw.Write((short)0x102);
                    for (j = 0; j < 10; j++)
                    {
                        binw.Write(0);
                    }
            ...
            int lastpart = mymesh.objcount * 64;
            int lptexs = 0;
            for (i = 0; i < mymesh.objcount; i++)
            {
                ...
                if (lptexs != 0)
                {
                    if (lptexs == mats[i].texs.Length)
                        lastpart -= 0x40;
                    else if (lptexs < mats[i].texs.Length)
                        lastpart -= 0x30;
                    else
                        lastpart -= 0x34;
                }
                lptexs = mats[i].texs.Length;
            ...
            binw.Write(0);
            binw.Write(12);
            binw.Write(12 + (texs.Count * 16));
            for (i = 0; i < texs.Count; i++)
            {
                binw.Write((texs.Count * 4) + i * 12);
            }
            ...
            for (i = 0; i < mymesh.objcount; i++)
            {
                ...
                for (j = 0; j < mymesh.objs[i].vertcount; j++)
                {
                    ...
                    if (mymesh.objs[i].uvs == 2)
                    {
                        binw.Write(mymesh.objs[i].verts[j].u2);
                        binw.Write(mymesh.objs[i].verts[j].v2);
                    }
                }
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                else
                {
                    ...
                    for (int j = 0; j < crowd.sides[i].quads.Length; j++)
                    {
                        ...
                        foreach (int k in ides)
                        {
                            outread.Write(crowd.sides[i].quads[j].verts[k].u);
                            outread.Write(crowd.sides[i].quads[j].verts[k].v);
                        }
                    }
